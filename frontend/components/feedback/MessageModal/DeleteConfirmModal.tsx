...
export default function DeleteConfirmModal({ isOpen, onClose, onConfirm, title }: DeleteConfirmModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-neutral-800 p-6 rounded-2xl w-full max-w-sm shadow-xl">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onConfirm} className="flex-1 py-2 rounded-xl bg-red-600 text-white">حذف نهایی</button>
        </div>
      </div>
    </div>
  );
